    ...
    for i in range(1, n_classes):
        ...
        for k, idx in enumerate(indices):
            if this_cls_gt[idx] == 1:
                ...
                psum += tp / (tp + fp)
                cpsum += wtp / (wtp + fp)
        this_cls_ap = psum/np.sum(this_cls_gt) if np.sum(this_cls_gt) > 0 else 0 #! added if
        this_cls_acp = cpsum / np.sum(this_cls_gt) if np.sum(this_cls_gt) > 0 else 0 #! added if

        all_cls_ap.append(this_cls_ap)
        all_cls_acp.append(this_cls_acp)

    map = sum(all_cls_ap) / len(all_cls_ap) if len(all_cls_ap) > 0 else 0 #! added if
    cap = sum(all_cls_acp) / len(all_cls_acp) if len(all_cls_acp) > 0 else 0 #! added if
...
class SmoothedValue(object):
    ...
    @property
    def global_avg(self):
        return self.total / self.count if self.count > 0 else self.total/(self.count+1) #! self.total / self.count
